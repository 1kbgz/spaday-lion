...
/* spaday-lion authored from Python, on a page it shares with a downstream library that imports
 * Lion by its bare specifiers. See spaday_lion/tests/integration.py.
 */

const PAGE = "http://127.0.0.1:8020";
...
test("a library importing Lion by name gets the page's copy", async ({
  page,
}) => {
  const errors = [];
  ...
  await expect(downstream.locator("lion-button")).toBeAttached();
