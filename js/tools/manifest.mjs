...
const PACKAGES = ["@lion/ui"];
const OUT = "../spaday_lion/custom-elements.json";
// Lion's manifest also picks up test fixtures, Storybook helpers and names read off variables
// (`tagString`); the define modules are the elements Lion actually registers, so only those are kept.
const REGISTERED = new Set(
  fs
    .readdirSync("node_modules/@lion/ui/exports/define")
    .map((file) => file.replace(/\.js$/, "")),
);
// and two it names wrongly: one wrapped in backticks, and LionSelectRich under a tag from a doc
// example rather than the lion-select-rich its define module registers
const RENAMED = {
  "`lion-input-range`": "lion-input-range",
  "lion-field-with-select": "lion-select-rich",
};
...
const UNPUBLISHED = fs.existsSync("tools/unpublished.json")
  ...
  : {};
// platform and Lit bases, and the helper Lion builds its mixins with, contribute nothing an author sets
