...
await micropip.install(wheels["lion"], deps=False)

example = importlib.import_module(f"spaday_lion.{example_name}")
...
async def call_endpoint(path, body_json):
    ...
    if path == "/api/applications":
        response = await example.apply(LocalRequest(body))
    elif match := re.fullmatch(r"/api/applications/([^/]+)/approve", path):
        response = await example.approve(LocalRequest(path_params={"id": match.group(1)}))
    else:
...
if hasattr(example, "review_applications"):
    asyncio.create_task(example.review_applications())
