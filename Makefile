...
lint-py:  ## run python linter with ruff
	python -m ruff check spaday_lion js/examples/build_pyodide_example.py
	python -m ruff format --check spaday_lion js/examples/build_pyodide_example.py
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_lion
	python -m ruff format spaday_lion
...
test-py:  ## run python tests
	python -m pytest -v spaday_lion/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_lion/tests --cov=spaday_lion --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	test -n "$(firstword $(wildcard dist/spaday_lion-*.whl))"
	...
	python js/examples/build_pyodide_example.py dist/lite "$(firstword $(wildcard dist/spaday_lion-*.whl))" dist/pyodide-deps
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_LION_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
